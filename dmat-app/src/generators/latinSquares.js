// Latin Squares Question Generator
// Based on dMAT Core Module - Latin Squares subtest
// Generates valid 5x5 Latin squares with 100% mathematically verified uniqueness
// and human-like step-by-step logical deduction paths (naked singles & hidden singles).
 ...
 * across all possible completions of the Latin square.
 * Returns false if ANY other letter in LETTERS can produce a valid Latin square.
 */
function isTargetUniquelyDetermined(initialGrid, targetRow, targetCol, expectedAnswer) {
  function countCompletions(g, pos) {
    ...
    for (const l of LETTERS) {
      if (!rowUsed.has(l) && !colUsed.has(l)) {
        ...
        g[r][c] = null;
        if (count > 0) return count; // early exit as soon as a completion is found
      }
    }
    return count;
  }
  ...
  for (const altLetter of LETTERS) {
    ...
    // If altLetter violates immediate row/column constraint, it cannot complete
    if (rowUsed.has(altLetter) || colUsed.has(altLetter)) continue;

    // Place altLetter in target cell and check if it can form ANY valid Latin square
...
export function generateLatinSquareQuestion(difficulty = 'medium', seed = null) {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    ...
    for (const [r, c] of shuffledCells) {
      ...
      // 1. Strict mathematical uniqueness: NO other letter can complete the grid
      if (!isTargetUniquelyDetermined(revealed, targetRow, targetCol, answer)) {
        revealed[r][c] = originalVal;
        continue;
      }

      // 2. Strict human deduction: Can target be solved by pure logic?
  ...
  // Safe fallback if loop didn't hit step range: guaranteed direct puzzle
  if (!bestPuzzle) {
    const fullGrid = generateCompleteLatin();
    const targetRow = Math.floor(Math.random() * 5);
    const targetCol = Math.floor(Math.random() * 5);
  ...
  const explanation = `Determine the letter for "?" at position ${targetCoord} (Row ${bestPuzzle.targetRow + 1}, Column ${greekTargetCol}). By systematically evaluating row and column constraints, the cell is uniquely forced to be "${correctAnswer}".`;
  const solutionSteps = bestPuzzle.steps.map((st, i) => `Step ${i + 1}: ${st.description}`);
